...
public class CourseEasyWalkSelector {
    ...
    public EasyWalkPlan select(FastPlan fastPlan) {
        ...
        for (PlannedStop stop : fastPlan.stops()) {
            ...
            List<WalkSelection> walkSelections = selectedTransit.walkSegments().stream()
    ...
    private WalkSelection selectWalk(
        ...
        boolean allProfiled = candidates.stream()
                .allMatch(candidate -> candidate.profile().isAvailable());
        Comparator<Candidate> comparator = allProfiled
                ? Comparator
        ...
        Candidate selected = candidates.stream()
                .min(comparator)
                .orElseThrow();
        WalkSelectionStatus status = allProfiled
                ? WalkSelectionStatus.PROFILED
                : WalkSelectionStatus.DEM_FALLBACK;
        ElevationProfileService.ProfileResult selectedProfile = allProfiled
                ? selected.profile()
                : ElevationProfileService.ProfileResult.unavailable();
        return new WalkSelection(
                segment,
