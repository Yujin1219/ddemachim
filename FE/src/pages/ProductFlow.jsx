...
const images = {
  ...
  map: '/assets/figma/map-background.png',
  navigation: '/assets/figma/navigation-map.png',
  onsite: '/assets/figma/onsite-media.png',
  complete: '/assets/figma/complete-map.png',
...
function MapStage({ children, variant = 'home' }) {
  const source = variant === 'complete' ? images.complete : variant === 'navigation' ? images.navigation : images.map;
...
function RouteOverview({ go, active = false }) {
  ...
  return <section className="phone standard-screen route-overview-screen"><main className="page-scroll route-overview-scroll"><header className="route-overview-heading"><IconButton label="이전" onClick={() => go(active ? 'progress' : 'compare')}>‹</IconButton><div><h1>{data.title}</h1><p>{data.subtitle}</p></div></header><section className="route-overview-map"><img src={images.navigation} alt="추천 경로 지도" /><RouteMotion /><span>{data.badge}</span><strong>{data.time}</strong></section><p className="route-overview-summary">{data.summary}</p><ScreenSection title="이동 순서" action="총 이동 52분"><ol className="route-overview-list">{data.stops.map(([name, stay, move, moveDetail], index) => <li key={`${name}-${index}`}><b>{index + 1}</b><div><strong>{name}</strong><span>{stay}</span>{move && <small><em>{move}</em>{moveDetail}</small>}</div></li>)}</ol></ScreenSection></main><div className="sticky-actions route-overview-actions"><ActionButton onClick={() => go(data.start)}>{active ? '이 코스로 계속하기' : '이 코스로 시작하기'}</ActionButton></div><BottomNav active="course" onNavigate={(tab) => go(rootRoutes[tab])} /></section>;
}
